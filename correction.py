...
_ARABIC_WORDLIST_URL = "https://raw.githubusercontent.com/linuxscout/arabicwordlists/master/arabic-wordlist-65k.txt"
_ARABIC_WORDLIST_CACHE_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "arabic-wordlist-65k.txt"
)


def _load_arabic_wordlist():
    """
    Loads the Arabic wordlist from a local cache file if present, otherwise
    downloads it once and caches it to disk. Falls back to an empty list
    (rather than crashing the whole app) if neither the cache nor the
    network is available — the Arabic spellchecker will simply have fewer
    known words in that degraded case.
    """
    if os.path.exists(_ARABIC_WORDLIST_CACHE_PATH):
        try:
            with open(_ARABIC_WORDLIST_CACHE_PATH, "r", encoding="utf-8") as f:
    ...
    try:
        response = requests.get(_ARABIC_WORDLIST_URL, timeout=15)
        ...
        try:
            with open(_ARABIC_WORDLIST_CACHE_PATH, "w", encoding="utf-8") as f:
                f.write(response.text)
        except Exception:
            # Caching is best-effort; failing to write to disk shouldn't
            # prevent the service from starting.
            pass

        return words
    except Exception:
        # No cache and no network — start with an empty base list rather
        # than taking the whole service down. common_arabic_corrections
        # and the LanguageTool fallback still provide some coverage.
        return []


arabic_words = _load_arabic_wordlist()
